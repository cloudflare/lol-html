define_state_group!(rcdata_states_group = {
    ...
    rcdata_state {
        b'<' => ( emit_chars; mark_tag_start; --> rcdata_less_than_sign_state )
        eoc  => ( emit_chars; )
    ...
    rcdata_less_than_sign_state {
        ...
        eof  => ( emit_chars; emit_eof; )
        _    => ( unmark_tag_start; emit_chars; reconsume in rcdata_state )
    }
    ...
    rcdata_end_tag_name_state {
        whitespace => (
            if is_appropriate_end_tag
                ...
                ( unmark_tag_start; emit_chars; reconsume in rcdata_state )
        )
