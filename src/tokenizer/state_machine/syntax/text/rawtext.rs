define_state_group!(rawtext_states_group = {
    ...
    rawtext_state {
        b'<' => ( emit_chars; mark_tag_start; --> rawtext_less_than_sign_state )
        eoc  => ( emit_chars; )
    ...
    rawtext_less_than_sign_state {
        ...
        eof  => ( emit_chars; emit_eof; )
        _    => ( unmark_tag_start; emit_chars; reconsume in rawtext_state )
    }
    ...
    rawtext_end_tag_name_state {
        whitespace => (
            if is_appropriate_end_tag
                ...
                ( unmark_tag_start; emit_chars; reconsume in rawtext_state )
        )
