...
macro_rules! action {
    ...
    ( | $self:tt, $ch:ident |> emit_chars ) => {
        if $self.pos > $self.raw_start {
            ...
            action_helper!(
                @emit_lex_unit_with_raw_exclusive |$self|> Some(ShallowToken::Character)
            );
        }
    };
    ...
    ( | $self:tt, $ch:ident |> emit_chars_up_to_cdata_end ) => {
        if $self.cdata_end_pos > $self.raw_start {
            action_helper!(
                @emit_lex_unit_with_raw |$self|> Some(ShallowToken::Character), $self.cdata_end_pos
            );
        }
    };
    ...
    ( | $self:tt, $ch:ident |> emit_current_token ) => {
        match $self.current_token.take() {
            Some(token) => {
                ...
                action_helper!(@emit_lex_unit_with_raw_inclusive |$self|> Some(token));
            }
