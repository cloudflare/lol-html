...
pub type DoctypeHandler<'h> = Box<dyn FnMut(&mut Doctype) -> HandlerResult + 'h + Send>;
/// Handler for HTML comments.
pub type CommentHandler<'h> = Box<dyn FnMut(&mut Comment) -> HandlerResult + 'h + Send>;
/// Handler for text chunks present the HTML.
pub type TextHandler<'h> = Box<dyn FnMut(&mut TextChunk) -> HandlerResult + 'h + Send>;
/// Handler for elements matched by a selector.
pub type ElementHandler<'h> = Box<dyn FnMut(&mut Element) -> HandlerResult + 'h + Send>;
/// Handler for an end tag.
pub type EndTagHandler<'h> = Box<dyn FnOnce(&mut EndTag) -> HandlerResult + 'h + Send>;
/// Handler for the document end, which is called after the last chunk is processed.
pub type EndHandler<'h> = Box<dyn FnOnce(&mut DocumentEnd) -> HandlerResult + 'h + Send>;
...
impl<'h> ElementContentHandlers<'h> {
    ...
    pub fn element(
        mut self,
        handler: impl FnMut(&mut Element) -> HandlerResult + 'h + Send,
    ) -> Self {
    ...
    pub fn comments(
        mut self,
        handler: impl FnMut(&mut Comment) -> HandlerResult + 'h + Send,
    ) -> Self {
    ...
    pub fn text(
        mut self,
        handler: impl FnMut(&mut TextChunk) -> HandlerResult + 'h + Send,
    ) -> Self {
...
impl<'h> DocumentContentHandlers<'h> {
    ...
    pub fn doctype(
        mut self,
        handler: impl FnMut(&mut Doctype) -> HandlerResult + 'h + Send,
    ) -> Self {
    ...
    pub fn end(
        mut self,
        handler: impl FnMut(&mut DocumentEnd) -> HandlerResult + 'h + Send,
    ) -> Self {
