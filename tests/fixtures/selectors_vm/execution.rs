use cool_thing::selectors_vm::{Ast, ElementData, MatchInfo, SelectorMatchingVm, VmError};
use cool_thing::{AuxStartTagInfo, EndTag, LocalName, MemoryLimiter, Namespace, StartTag};
        ...
        parse_token!($tag_html, UTF_8, StartTag, |t: &mut StartTag| {
...

            {
                ...
                if $expectation.should_bailout {
                    let aux_info_req = match result.expect_err("Bailout expected") {
                        VmError::InfoRequest(aux_info_req) => aux_info_req,
